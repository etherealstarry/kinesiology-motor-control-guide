...
**TAMU Kinesiology 常见 QE 考试形式（基于往年经验）**：

!!! tip "笔试部分（通常有）"
    - **形式**：Take-home 或 In-person 限时写作
    - **时长**：通常 1-2 周（Take-home）或 4-8 小时（In-person）
    - **内容**：
      - 从给定文献中提炼研究问题并设计实验
      - 解释统计结果（ANOVA 表、p 值、效应量）
      - 批判性评估一篇论文的方法和结论
      - 论述某个核心概念（如：内部模型、感觉权重、fMRI 与 fNIRS 的优劣）
    - **评分**：通常按 A/B/C/F 评级，需要委员会一致通过
...
!!! success "来自往届学生的备考建议"
    1. **提前 3-6 个月开始准备**
    2. **整理核心概念清单**（约 30-50 个，如：内部模型、感觉加权、fMRI vs fNIRS）
    3. **用 Anki 做闪卡**（间隔重复，记忆更牢固）
    4. **和同实验室学长/学姐 mock exam**（模拟口试，找差距）
    5. **重读经典论文**（Shadmehr & Krakauer 2008, Wolpert et al. 2001 等）
    6. **练习用英文解释公式**（如：导数、梯度下降、贝叶斯更新）

**推荐备考资源**：

| 资源 | 类型 | 说明 | 链接 |
|------|----------|----------|------|
| **How to Do Preliminary Exam (知乎)** | 中文经验 | 通用博士 QE 备考心法 | [知乎](https://zhuanlan.zhihu.com/p/463458252) |
| **Motor Control Lecture Notes (Shadmehr)** | 笔记 | 约翰霍普金斯课程笔记 | [Lab Site](https://www.shadmehrlab.org/) |
| **Anki 牌组：Motor Neuroscience** | 闪卡 | 自製，适合记忆核心概念 | 在 AnkiWeb 搜索 |
| **Past Exam Questions (TAMU KNSM)** | 经验 | 询问学长学姐获取往年题目 | 联系实验室 |
...
**题型 1：解释核心概念**（通常 2-3 题，每题 20-30 分）

> **示例题目**：
> - "解释内部模型（Internal Model）的概念，并说明正向模型与逆向模型的区别"
> - "什么是贝叶斯最优感知？用公式说明感觉加权的原理"

**应对策略**：
- 用 **"定义 + 例子 + 意义"** 三段式回答
- 画图！（如：内部模型的框图、贝叶斯加权的示意图）
- 引用经典论文（如：Wolpert et al. 1995）

---

**题型 2：设计实验**（通常 1-2 题，每题 30-40 分）

> **示例题目**：
> - "设计一个实验，检验'运动学习是否依赖睡眠'。说明被试、设备、实验流程、数据分析计划"
> - "如何证明某个脑区（如 M1）在运动学习中必要？设计因果推断实验"

**应对策略**：
- 用 **PICOS** 框架：被试（Participants）、干预（Intervention）、对照（Control）、结果（Outcomes）、研究设计（Study Design）
- 讨论**混淆变量**和**控制方法**
- 说明**预期结果**和**可能的替代解释**

---

**题型 3：批判性评估论文**（通常 1 题，30-40 分）

> **示例题目**：
> - "给定一篇论文（如：某 fMRI 研究），评估其方法是否严谨，结论是否合理"

**应对策略**：
- 用 **"优点 + 缺点 + 改进建议"** 三段式回答
- 重点关注：**样本量、控制组、统计方法、效应量、可重复性**
...
**准备策略**：
- 准备 **1 分钟 Elevator Pitch** + **5 分钟详细版**
- 用 **"背景 → 问题 → 方法 → 意义"** 结构
...
> **可能问题**：
> - "解释最优反馈控制（OFC）理论，并说明与传统轨迹规划模型的区别"
> - "解释 HKB 模型，并说明分岔的概念"

**准备策略**：
- 用 **Mermaid 画图**（在白板上画相空间、吸引子）
- 准备 **"如果...会怎样？"** 的反事实推理
...
!!! danger "往届学生的教训"
    1. **低估口试难度**：以为口试只是"聊天"，结果被问到不会的问题
       - ✅ 解决：提前 1 个月开始 mock oral exam，每周 1 次
    
    2. **笔试时间管理不当**：在一道题上花太多时间，导致后面题目没时间写
       - ✅ 解决：模拟考试时严格计时，每题分配固定时间
    
    3. **只复习知识点，不练习解释**：知道概念，但无法用英文清晰表达
       - ✅ 解决：用"费曼学习法"——假装给本科生讲课
    
    4. **忽略统计部分**：以为统计很简单，结果 ANOVA 表看不懂
       - ✅ 解决：复习 STAT 652 的所有作业，特别是 ANOVA 输出解读
    
    5. **没有准备"失败方案"**：如果实验失败，没有替代计划
       - ✅ 解决：在 Research Proposal 中准备 Plan B
